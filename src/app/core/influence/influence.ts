 ...
 * Uses each building's sightRange from BUILDING_STATS as the influence radius.
 */
export function computeInfluenceForPlayer(buildings: Map<string, BuildingData>, playerId: string): Set<string> {
  const result = new Set<string>();
  for (const building of buildings.values()) {
    if (building.ownerId !== playerId) continue;
    const range = BUILDING_STATS[building.type].sightRange;
